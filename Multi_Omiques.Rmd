...
path <- "/home/etudiant/Téléchargements"
setwd(path)
...
coef.protein <-  as.numeric(lapply(protein_data, var))
hist(coef.protein)
```

```{r}
protein_data_filtered <- protein_data[, abs(coef.protein) > 10]
```
...
```
```{r}
RNA_data_filtered <- RNA_data[, abs(coef.RNA) > 3]
```
...
mixOmics::plotIndiv(final.pca_protein,
                    ...
                    ind.names = TRUE,cex=3, group=cyto_data_1$Classe) # Montrez les noms des échantillons
