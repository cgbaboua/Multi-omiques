...
```{r}
pca_cyto <- mixOmics::pca(cyto_data2,ncomp = 10) #nrmlmt option scale=TRUE mais tu as déjà scale plus haut
plot(pca_cyto)
...
final.pca_cyto <- mixOmics::pca(cyto_data2, ncomp = 5, center = TRUE) 
...
pca_protein <- mixOmics::pca(protein_data, ncomp = 10) 
...
final.pca_protein <- mixOmics::pca(protein_data, ncomp = 5, center = TRUE) 
...
mixOmics::plotVar(final.pca_protein)
```
...
pca_rna <- mixOmics::pca(rna_data, ncomp = 10)
plot(pca_rna)
...
final.pca_rna <- mixOmics::pca(rna_data, ncomp = 5, center = TRUE) 
...
mixOmics::plotVar(final.pca_rna)
```
